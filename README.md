...
В пакете `internal/api` каждый HTTP-сценарий находится в отдельном файле вместе со своими DTO и обработчиком. Преобразования доменных моделей в успешные HTTP-ответы собраны в `presenter.go`. Создание сервера, регистрация маршрутов и общий формат ошибок вынесены отдельно:
...
| [`presenter.go`](internal/api/presenter.go) | Преобразование доменных моделей в DTO успешных HTTP-ответов. |
...
Go компилирует все файлы `.go`, не относящиеся к тестам и объявляющие `package api`, как единый пакет. Поэтому обработчики могут вызывать закрытые функции `newCreateTripResponse` и `newGetTripByIDResponse` из `presenter.go`, а также `writeFiberServiceError` из `error_response.go`. Успешные DTO остаются в файлах соответствующих ручек, чтобы контракты создания и получения поездки могли изменяться независимо, а функции их преобразования собраны в `presenter.go` как отдельная ответственность слоя API. По аналогии `helpers_test.go` содержит функции, общие для нескольких тестов, и благодаря суффиксу `_test.go` компилируется только при запуске тестов.
